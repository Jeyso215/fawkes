"""
Tensorflow implementations of models trained with ArcFace loss.
We substitute Fawkes's original face feature extractors with this one to test our attack on VeriLight.

From https://github.com/leondgarse/Keras_insightface/tree/master
...
basic_model = buildin_models("ResNet101V2", dropout=0.4, emb_shape=512, output_layer="E")
# embs = []
# for i in range(1, 4):
#     print(i)
#     img = cv2.imread(f"lfw/A/Amanda_Bynes_000{i}.jpg")
#     img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
#     # resize to 112x112
#     img = cv2.resize(img, (112, 112))
#     img = (img- 127.5) * 0.0078125
#     emb = basic_model(img[None, ...])
#     emb = emb.numpy()
#     emb = emb / np.linalg.norm(emb)
#     embs.append(emb)
# # compute the distance between all pairs of embeddings
# distances = []
# for i in range(3):
#     for j in range(3):
#         if i == j:
#             continue
#         distance = np.arccos(np.dot(embs[i][0], embs[j][0]))
